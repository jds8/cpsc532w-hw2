...
class Gamma(dist.Gamma):
    ...
    def __init__(self, concentration, rate, fixed=""):
    ...
    def make_copy_with_grads(self):
        ...
        if self.fixed == "":
            ...
            concentration = [p.clone().detach().requires_grad_() for p in self.Parameters()][0]
            rate = self.optim_rate
        else:
            concentration = self.concentration
            rate = [p.clone().detach().requires_grad_() for p in self.Parameters()][0]

        return Gamma(concentration, rate, self.fixed)
